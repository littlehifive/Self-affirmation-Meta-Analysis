...
# {https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/pcurve.html}{Chapter 9.2}.
...
# fail safe test
failsafe.test <- function(data,level,type){
  ...
  if(type == "Rosenthal"){
  output <- data.frame(type = test$type,
                       N = test$fsnum,
                       pval = test$pval,
                       alpha = test$alpha)
  } else {
    output <- data.frame(type = test$type,
                         N = test$fsnum,
                         average_ES = test$meanes,
                         target_ES = test$target)
  }
